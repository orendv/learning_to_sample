...
parser.add_argument('--classifier_model', default='pointnet_cls_basic', help='Classifier model name [pointnet_cls/pointnet_cls_basic] [default:pointnet_cls]')
...
parser.add_argument('--data_path', default='log/ProgressiveNet/sampled', help='path to a folder containing .h5 files with sampled point clouds ')
...
parser.add_argument('--min_pc_size', type=int, default=2, help='Minimal sample size to evaluate on [default:2]')
...
def evaluate(pc_size):
    with tf.device('/gpu:' + str(GPU_INDEX)):
        pointclouds_pl, labels_pl = CLASSIFIER_MODEL.placeholder_inputs(BATCH_SIZE, pc_size)
...
def eval_one_epoch(sess, ops, pc_size, topk=1):
    ...
    log_string('sample size: %d, eval accuracy: %f' % (pc_size, total_correct / float(total_seen)))


if __name__ == '__main__':
    if DENSE_EVAL:
        pc_sizes = reversed(range(MIN_PC_SIZE, MAX_PC_SIZE+1))
    else:
