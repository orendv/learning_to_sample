...
if flags.use_fps:
    ...
    file_name = "_".join(["sampled_pc", "test_set", flags.object_class, "fps", "%04d" % n_sample_points]) + ".npy"
    file_path = osp.join(eval_dir, file_name)
    np.save(file_path, sampled_pc)

    file_name = "_".join(["sample_idx", "test_set", flags.object_class, "fps", "%04d" % n_sample_points]) + ".npy"
    file_path = osp.join(eval_dir, file_name)
...
file_name_wo_fps = "_".join(["reconstructions", "test_set", flags.object_class]) + ".npy"
if flags.use_fps:
    file_name = "_".join(["reconstructions", "test_set", flags.object_class, "fps", "%04d" % n_sample_points]) + ".npy"
else:
...
if flags.use_fps:
    file_name = "_".join(["ae_loss", "test_set", flags.object_class, "fps", "%04d" % n_sample_points]) + ".npy"
else:
    file_name = file_name_wo_fps
...
if flags.use_fps:
    log_file_name = "_".join(["eval_stats", "test_set", flags.object_class, "fps", "%04d" % n_sample_points]) + ".txt"
else:
