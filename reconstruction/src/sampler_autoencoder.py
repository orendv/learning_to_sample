...
class SamplerAutoEncoder(Neural_Net):
    ...
    def get_critical_idx(self, S):
        '''Get indices of critical points.'''
        return self.sess.run(self.critical_idx, feed_dict={self.s: S})
    ...
    def evaluate(self, in_data, configuration, ret_pre_augmentation=False):
        ...
        critical_idxs_samp_fps = self.get_critical_idxs(samp_fps, b)
        ...
        for i in xrange(0, n_examples, b):
            if self.is_denoising:
                recon_gen[i:i + b] = self.reconstruct(feed_data[i:i + b], original_data[i:i + b], S=samp_gen[i:i + b], compute_loss=False)[0]
                recon_prj[i:i + b] = self.reconstruct(feed_data[i:i + b], original_data[i:i + b], S=samp_prj[i:i + b], compute_loss=False)[0]
                recon_fps[i:i + b] = self.reconstruct(feed_data[i:i + b], original_data[i:i + b], S=samp_fps[i:i + b], compute_loss=False)[0]
            else:
        ...
        ret = {'feed_data': feed_data, 'ids': ids, 'original_data': original_data, 'pre_aug': pre_aug,
               ...
               'samp_prj_idx': samp_prj_idx, 'n_unique_prj': n_unique_prj, 'critical_idxs_samp_fps': critical_idxs_samp_fps,
    ...
    def get_critical_idxs(self, pclouds, batch_size=100):
        ''' Convenience wrapper of self.get_cirtical_idx to get the critical indices for a set of input point clouds.
        Args:
            pclouds (N, K, 3) numpy array of N point clouds with K points each.
            batch_size size of point clouds batch
        '''
        ...
        idx = np.arange(len(pclouds))
        for b in iterate_in_chunks(idx, batch_size):
            critical_curr = self.get_critical_idx(pclouds[b])
            critical_idxs.append(critical_curr)
        return np.vstack(critical_idxs)
