...
class SampleNetPointNetAutoEncoder(SamplerAutoEncoder):
    '''
    ...
    def __init__(self, sampler_name, configuration, graph=None):
        ...
        with tf.variable_scope(sampler_name):
            ...
            self.s = c.sampler(self.x, c.n_samp)

        with tf.variable_scope(c.ae_name):
            self.z, self.s_features = c.encoder(self.s, **c.encoder_args)
            self.critical_idx = tf.arg_max(self.s_features, dimension=1)
    ...
    def _create_loss(self):
        ...
        # sample loss
        self.loss_similarity, self.dist, self.idx, self.dist2 = self._get_similarity_loss(self.x, self.s, c.n_samp[0])
        ...
        if c.similarity_reg_weight > 0.0:
            self.loss = self.loss_ae + c.similarity_reg_weight * self.loss_similarity
        else:
