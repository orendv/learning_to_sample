...
class PointNetAutoEncoder(AutoEncoder):
    ...
    def __init__(self, name, configuration, graph=None):
        ...
        with tf.variable_scope(name):
            ...
            # sort output by FPS indices
            n_output_points = self.n_output[0]
            idx_fps = farthest_point_sample(n_output_points, self.x_reconstr)  # (batch_size, n_output_points)
            self.x_reconstr_sorted = gather_point(self.x_reconstr, idx_fps)  # (batch_size, n_output_points, 3)

            # chamfer distance between a sample of the output and ground truth data
            self._create_nn_distance()
    ...
    def _create_nn_distance(self):
        cost_p1_p2, _, cost_p2_p1, _ = nn_distance(self.s_out, self.gt)
        self.nn_distance = tf.reduce_mean(cost_p1_p2, axis=1, keep_dims=True) + tf.reduce_mean(cost_p2_p1, axis=1, keep_dims=True)
